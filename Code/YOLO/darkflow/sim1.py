...
parser = argparse.ArgumentParser(description="Set the random seed for simulation.")
parser.add_argument(
    "--seed", 
    type=int, 
    default=0,  # Default seed value
    help="Specify the random seed (default: 0)"
)
        ...
        for j in range(len(vehicles[directionNumbers[nextGreen]][i])):
            ...
            if(vehicle.crossed==0):
                ...
                if(vclass=='car'):
                    ...
                    noOfRickshaws += 1
    # print(noOfCars)
    greenTime = 20#math.ceil(((noOfCars*carTime) + (noOfRickshaws*rickshawTime) + (noOfBuses*busTime) + (noOfTrucks*truckTime)+ (noOfBikes*bikeTime))/(noOfLanes+1))
